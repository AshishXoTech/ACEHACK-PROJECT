...
const CRITERIA: {
  key: keyof Omit<EvaluationPayload, "comments">;
  label: string;
  desc: string;
}[] = [
  { key: "innovation", label: "Innovation Score", desc: "Originality and creative problem solving" },
  { key: "technical", label: "Technical Score", desc: "Depth and quality of technical implementation" },
  { key: "impact", label: "Impact Score", desc: "Real-world applicability and potential impact" },
  { key: "presentation", label: "Presentation Score", desc: "Clarity of communication and demo quality" },
];

type Scores = Omit<EvaluationPayload, "comments">;
...
export default function EvaluatePage() {
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      await judgeService.submitEvaluation(teamId, { ...scores, comments });
      setDone(true);
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white">
      <div className="max-w-2xl mx-auto p-10">
        ...
        <p className="text-gray-400 text-sm mb-8">Team ID: {teamId}</p>

        <div className="bg-[#0f172a] border border-slate-800 rounded-xl p-6 mb-6 flex items-center justify-between">
          <span className="text-gray-400 text-sm">Calculated Total Score (4 criteria)</span>
        ...
        <div className="space-y-5 mb-6">
          {CRITERIA.map((criterion) => (
            <div key={criterion.key} className="bg-[#0f172a] border border-slate-800 rounded-xl p-5">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <div className="font-medium text-white">{criterion.label}</div>
                  <div className="text-gray-500 text-xs mt-0.5">{criterion.desc}</div>
                </div>
                <span className="text-2xl font-bold text-blue-400">{scores[criterion.key]}</span>
              </div>
              ...
              <input
                ...
                onChange={(e) =>
                  setScores((prev) => ({ ...prev, [criterion.key]: Number(e.target.value) }))
                }
